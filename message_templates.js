...
export function generateCategoryFlexMessage(categories) {
  ...
  const bubbles = categories.map(category => {
    ...
    return {
      ...
      footer: {
        type: "box", layout: "vertical", paddingAll: "xs", contents: [
...
export function generateCourseFlexMessage(courses) {
  ...
  const bubbles = courses.slice(0, 10).map(course => {
    let img = course.imageUrl || "https://scdn.line-apps.com/n/channel_devcenter/img/fx/01_1_cafe.png";
      ...
      body: {
        type: "box", layout: "vertical", paddingAll: "0px", contents: [
          ...
          { type: "box", layout: "vertical", paddingAll: "lg", spacing: "sm", contents: [
            { type: "text", text: course.name, weight: "bold", size: "xl", wrap: true },
            { type: "text", text: course.description || "暫無簡介", size: "sm", color: "#666666", wrap: true, maxLines: 5 },
            { type: "text", text: `NT $${course.price}起`, color: "#FF0000", align: "end", weight: "bold", size: "lg", margin: "md" }
          ]}
        ]
      },
      footer: {
        type: "box", layout: "vertical", spacing: "sm", contents: [
          ...
          { type: "box", layout: "horizontal", contents: [
            { type: "button", action: { type: "uri", label: "課程說明 >", uri: detailUri }, style: "link", height: "sm" },
  ...
  const bubbles = orders.map(order => ({
    ...
    body: {
      type: "box", layout: "vertical", spacing: "md", contents: [
        ...
        { type: "separator" },
        { type: "box", layout: "vertical", spacing: "sm", contents: [
          { type: "text", text: order.courseName, weight: "bold", size: "sm", wrap: true },
          { type: "text", text: `訂單編號: ${order.orderId}`, size: "xs", color: "#aaaaaa" },
          { type: "text", text: `應付金額: NT$ ${order.amount}`, size: "sm", color: "#FF0000", weight: "bold" },
          { type: "text", text: `目前狀態: ${order.status}`, size: "sm", weight: "bold" }
        ]},
        { type: "box", layout: "vertical", backgroundColor: "#f8f8f8", paddingAll: "md", cornerRadius: "sm", contents: [
          { type: "text", text: "匯款帳戶：(822) 中國信託", size: "xs", color: "#888888" },
          { type: "text", text: "帳號：123-45678-9012", size: "xs", color: "#888888" },
          { type: "text", text: "匯款後請點擊下方回報", size: "xs", color: "#888888", margin: "xs" }
        ]}
      ]
    },
    footer: {
      type: "box", layout: "vertical", spacing: "sm", contents: [
        { type: "button", action: { type: "message", label: "回報匯款", text: `我想回報匯款 (單號:${order.orderId})` }, style: "primary", height: "sm" },
        { type: "button", action: { type: "message", label: "取消報名", text: `我想取消報名 (單號:${order.orderId})` }, style: "secondary", height: "sm" }
      ]
    }
  }));
  return { type: "flex", altText: "您的報名紀錄清單", contents: { type: "carousel", contents: bubbles } };
}
