...
export async function handleAdminPage(env) {
  const h = [
    ...
    '<h1 class="text-2xl font-bold mb-1 tracking-tight">人生進化 Action</h1><p class="text-slate-400 text-sm mb-8 uppercase font-medium">Administrator</p>',
    ...
    'const b=document.getElementById("lbtn"); b.innerText="驗證中...";',
    'try{',
    '// 【修復】改呼叫 /api/admin/ 以執行 Worker 層級認證',
    ...
    'else{alert("帳號或密碼錯誤 (401)");}',
    '}catch(e){alert("解析資料失敗："+t.substring(0,50));}',
    '}catch(e){alert("無法連接 API 代理伺服器");}finally{b.innerText="進入管理系統";}',
    '}',
    'function st(t){ State.tab=t; render(); }',
    'function render(){ const s=document.getElementById("main-section"); s.innerHTML="<p class=\'p-4\'>目前功能統計：已連線至資料庫</p>"; }',
    '</script></body></html>'
  ].join("\n");
