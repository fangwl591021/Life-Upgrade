...
export async function handleAdminPage(env) {
  const h = [
    ...
    '<title>管理後台 - Action Pro</title><script src="https://cdn.tailwindcss.com"></script>',
    ...
    'else{alert("帳號或密碼錯誤");}',
    '}catch(e){d.classList.remove("hidden"); d.innerText="連線成功但格式不對。請重新部署 GAS。\\n內容："+t.substring(0,200);}',
    ...
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden font-sans"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b border-slate-100"><tr><th class="p-5 text-base text-slate-400">ID</th><th class="p-5 text-base text-slate-400">學員資訊</th><th class="p-5 text-base text-slate-400">課程</th><th class="p-5 text-base text-slate-400">實收</th><th class="p-5 text-center text-base text-slate-400">狀態</th><th class="p-5 text-right font-semibold text-base text-slate-400">管理</th></tr></thead><tbody class="text-sm">\'+rows+\'</tbody></table></div>\';',
