...
export async function handleAIRequest(event, env) {
  ...
  if (orderMatch) {
    ...
    try {
      ...
      return await callDualEngineAI(event, "您的報名已在處理中，但目前卡片獲取稍慢，請輸入「我的預約」查詢結果。", env);
    }
  }

  // 意圖：我的預約查詢
  if (userMessage.includes("我的預約") || userMessage.includes("查詢紀錄")) {
    const orders = await getUserOrders(userId, env);
