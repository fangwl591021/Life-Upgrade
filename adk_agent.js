...
export async function handleAIRequest(event, env) {
  ...
  if (orderMatch) {
    ...
    try {
      ...
      return await replyToLINE(event.replyToken, "感謝預約！✨ 請點擊下方按鈕完成匯款回報。", generateOrderListFlexMessage(orders), env);
    } catch (e) { return await replyToLINE(event.replyToken, "系統忙碌中，請稍後再試。", null, env); }
  }

  // 2. 識別分類查詢意圖 (解決 image_ecf4fa.png 的毛病)
  ...
  // 4. AI 雙引擎回應 (嚴格控制：只有在無法匹配上述意圖時才呼叫 AI)
  const systemPrompt = "你是人生進化 Action 專業客服。嚴格禁止虛構課程。只能針對使用者提供的課程資料回答。格式簡潔親切，不加粗，不包框。";
  ...
  await replyToLINE(event.replyToken, "系統無法解析您的需求，請稍後再試，或直接點擊選單查詢。", null, env);
}
