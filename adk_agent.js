...
export async function handleAIRequest(event, env) {
  ...
  if (orderMatch) {
    ...
    try {
      ...
      // 成功後立即 return，防止後方的 AI 引擎執行
      return await replyToLINE(event.replyToken, "感謝您的預約！✨ 請點擊下方按鈕完成匯款回報。", generateOrderListFlexMessage(orders), env);
    } catch (e) {
      // 備援方案：若發生異常，嘗試用 GPT-4o 生成帶有人情味的安撫
      return await callDualEngineAI(event, "報名資料已寫入但確認卡片獲取失敗，請引導使用者過幾分鐘輸入「我的預約」查看。", env);
    }
  }
...
async function callDualEngineAI(event, userMessage, env) {
  const systemPrompt = "你是專業客服。嚴格禁止虛構課程資料。模擬原生資訊流，不加粗、不包框。";
