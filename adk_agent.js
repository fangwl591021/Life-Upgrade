...
export async function handleAIRequest(event, env) {
  ...
  if (categoryMatch) {
    ...
    return await replyToLINE(event.replyToken, "抱歉，目前找不到該分類的課程。", null, env);
  }

  // 4. AI 客服
  const systemPrompt = "你是專業客服。嚴格禁止虛構課程。只能引導點擊選單查詢。不加粗不包框。";
  ...
  await replyToLINE(event.replyToken, "抱歉，系統暫時無法解析需求。", null, env);
}
