...
export async function handleAIRequest(event, env) {
  ...
  const workerBase = "https://lifeupgrade.fangwl591021.workers.dev"; // 修正後的基礎網址

  // 1. 處理報名指令 (同步修正網址至 /pay)
  ...
  if (orderMatch) {
    ...
    try {
      ...
      // 注意：這裡 Flex Message 中的 URI 需要改為 /pay，在模組中處理
      return await replyToLINE(event.replyToken, '感謝您的預約！✨ 請點擊下方按鈕完成匯款回報 💳', generateOrderListFlexMessage(orders, workerBase), env);
  ...
  if (userMessage.includes('我的預約') || userMessage.includes('我的報名')) {
    const orders = await getUserOrders(userId, env);
    if (orders && orders.length > 0) return await replyToLINE(event.replyToken, '這是您的報名紀錄：', generateOrderListFlexMessage(orders, workerBase), env);
  ...
  // 6. 雙引擎 AI
  const systemPrompt = "你是專業客服。模擬 LINE 原生資訊流格式，不包框、不加粗、親切精確。";
