...
export async function handleAIRequest(event, env) {
  ...
  // 1. 識別報名預約指令 (Regex 強阻斷，確保絕對 return)
  ...
  if (orderMatch) {
    ...
    try {
      ...
      // 成功後立即 return，防止後方的 AI 引擎執行發送閒聊
      return await replyToLINE(event.replyToken, "感謝您的預約！✨ 請點擊下方按鈕完成匯款回報。", generateOrderListFlexMessage(orders), env);
    } catch (e) {
      // 發生錯誤，呼叫備援機制
      return await callDualEngineAI(event, "您的報名已在處理中，但目前獲取預約卡片稍慢，請稍後輸入「我的預約」查看。", env);
    }
  }
...
async function callDualEngineAI(event, userMessage, env) {
  const systemPrompt = "你是人生進化 Action 專業客服。嚴格禁止虛構課程資料。模擬 LINE 原生資訊流，不加粗、不包框。";
  ...
  await replyToLINE(event.replyToken, "系統目前稍嫌忙碌，請稍後再試。", null, env);
}
