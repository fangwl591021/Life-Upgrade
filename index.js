...
function handleStatusPage() {
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Status</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-slate-50 flex items-center justify-center min-h-screen font-sans"><div class="max-w-md w-full bg-white p-10 rounded-3xl shadow-xl text-center"><h1 class="text-2xl font-black mb-6">人生進化 Action</h1><a href="/admin" class="block bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-100">前往管理後台</a></div></body></html>';
  return new Response(html, { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
}

async function handleAdminPage(env) {
  const html = '<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><title>Sportsland Pro</title><script src="https://cdn.tailwindcss.com"></script><style>body{font-family:sans-serif;background:#f8fafc}.nav-active{color:#2563eb;border-bottom:2px solid #2563eb;font-weight:600}</style></head><body><div id="login-box" class="fixed inset-0 bg-slate-50 flex items-center justify-center z-50"><div class="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl"><h1 class="text-2xl font-black text-center mb-6">管理系統認證</h1><input type="text" id="admin-user" class="w-full border p-3 rounded-xl mb-4 outline-none" placeholder="Username"><input type="password" id="admin-pw" class="w-full border p-3 rounded-xl mb-6 outline-none" placeholder="Password"><button onclick="doLogin()" id="lbtn" class="w-full bg-blue-600 text-white p-4 rounded-xl font-bold">登入系統</button><div id="diag" class="hidden mt-4 p-3 bg-red-50 text-red-600 text-[10px] rounded font-mono overflow-auto max-h-40"></div></div></div><div id="app-box" class="hidden"><header class="bg-white border-b sticky top-0 px-4 h-16 flex items-center justify-between z-10"><span class="font-bold text-blue-600">Sportsland Admin</span><nav class="flex space-x-6"><button onclick="st(\'dashboard\')" id="n-dashboard" class="nav-active text-sm py-5">數據看板</button><button onclick="st(\'orders\')" id="n-orders" class="text-slate-500 text-sm py-5">訂單管理</button><button onclick="st(\'courses\')" id="n-courses" class="text-slate-500 text-sm py-5">課程維護</button><button onclick="st(\'users\')" id="n-users" class="text-slate-500 text-sm py-5">學員名單</button></nav><button onclick="location.reload()" class="text-xs text-slate-400">登出</button></header><main class="max-w-7xl mx-auto p-6" id="cnt"></main></div><script>let u="",p="",sd=null;async function doLogin(){u=document.getElementById(\'admin-user\').value;p=document.getElementById(\'admin-pw\').value;const b=document.getElementById(\'lbtn\');const d=document.getElementById(\'diag\');b.innerText="驗證中...";d.classList.add(\'hidden\');try{const r=await fetch(\'/api/admin/adminGetData\',{headers:{\'X-Admin-User\':u,\'X-Admin-Pass\':p}});const t=await r.text();try{const j=JSON.parse(t);if(r.ok&&j.status===\'success\'){sd=j;document.getElementById(\'login-box\').classList.add(\'hidden\');document.getElementById(\'app-box\').classList.remove(\'hidden\');st(\'dashboard\')}else{alert(\'認證失敗\')}}catch(e){d.classList.remove(\'hidden\');d.innerText="🚨收到 HTML 錯誤，請檢查 GAS 權限設為所有人。\\n內容："+t.substring(0,300)}}catch(e){alert(\'連線失敗\')}finally{b.innerText="登入系統"}}function st(tab){document.querySelectorAll(\'nav button\').forEach(b=>b.className="text-slate-500 text-sm py-5");document.getElementById(\'n-\'+tab).className="nav-active text-sm py-5";const d=sd.data;if(tab===\'dashboard\'){const pend=d.orders.filter(o=>o.status===\'待匯款\').length;const inc=d.orders.filter(o=>o.status===\'已確認\').reduce((s,o)=>s+(parseInt(o.amount)||0),0);document.getElementById(\'cnt\').innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-3 gap-6"><div class="bg-white p-6 rounded-2xl shadow-sm"><div class="text-slate-400 text-xs font-bold mb-2 uppercase">待處理訂單</div><div class="text-3xl font-black text-orange-500">\'+pend+\' 筆</div></div><div class="bg-white p-6 rounded-2xl shadow-sm"><div class="text-slate-400 text-xs font-bold mb-2 uppercase">累計總收入</div><div class="text-3xl font-black text-emerald-600">$\'+inc.toLocaleString()+\'</div></div><div class="bg-white p-6 rounded-2xl shadow-sm"><div class="text-slate-400 text-xs font-bold mb-2 uppercase">總學員數</div><div class="text-3xl font-black text-blue-600">\'+d.users.length+\' 位</div></div></div>\'}if(tab===\'orders\'){let rs=d.orders.map(o=>\'<tr class="border-b text-sm"><td class="p-4 font-mono text-xs text-slate-400">\'+o.orderId+\'</td><td class="p-4 font-bold">\'+(o.name||\'-\')+\'</td><td class="p-4">\'+o.courseName+\'</td><td class="p-4 font-bold text-blue-600">$\'+o.amount+\'</td><td class="p-4 text-xs">\'+o.status+\'</td><td class="p-4 font-mono text-blue-500">\'+(o.last5||\'-\')+\'</td><td class="p-4 text-right"><button onclick="uo(\\\'\'+o.orderId+\'\\\',\\\'已確認\\\')" class="text-blue-600 font-bold mr-3 text-xs uppercase">確認</button><button onclick="uo(\\\'\'+o.orderId+\'\\\',\\\'已取消\\\')" class="text-slate-300 font-bold text-xs uppercase">取消</button></td></tr>\').join(\'\');document.getElementById(\'cnt\').innerHTML=\'<div class="bg-white rounded-2xl shadow-sm overflow-hidden"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[10px] text-slate-400 font-bold uppercase"><th class="p-4">Order ID</th><th class="p-4">Name</th><th class="p-4">Course</th><th class="p-4">Amount</th><th class="p-4">Status</th><th class="p-4">Bank</th><th class="p-4">Action</th></tr></thead><tbody>\'+rs+\'</tbody></table></div>\'}if(tab===\'courses\'){let cs=d.courses.map(c=>\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-transparent hover:border-blue-100 transition"><div class="flex justify-between mb-4"><div class="font-bold">\'+c.name+\'</div><div class="text-[10px] font-mono text-slate-300">\'+c.id+\'</div></div><div class="space-y-4"><div><label class="text-[10px] text-slate-400 font-bold uppercase">類別</label><input id="c-\'+c.id+\'" value="\'+c.category+\'" class="w-full border-b text-xs py-1 outline-none"></div><div><label class="text-[10px] text-slate-400 font-bold uppercase">價格</label><input id="p-\'+c.id+\'" type="number" value="\'+c.price+\'" class="w-full border-b text-lg font-black text-blue-600 outline-none"></div><button onclick="sc(\\\'\'+c.id+\'\\\')" class="w-full bg-slate-900 text-white py-2 rounded-xl text-xs font-bold">儲存更新</button></div></div>\').join(\'\');document.getElementById(\'cnt\').innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-3 gap-6">\'+cs+\'</div>\'}if(tab===\'users\'){let us=d.users.map(u=>\'<tr class="border-b hover:bg-slate-50 transition"><td class="p-4 text-xs font-mono text-slate-400">\'+u.uid+\'</td><td class="p-4 font-bold">\'+u.name+\'</td><td class="p-4 text-sm font-mono text-slate-500">\'+u.phone+\'</td><td class="p-4 text-xs text-slate-300">\'+u.time+\'</td></tr>\').join(\'\');document.getElementById(\'cnt\').innerHTML=\'<div class="bg-white rounded-2xl shadow-sm overflow-hidden"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[10px] text-slate-400 font-bold uppercase"><th class="p-4">LINE UID</th><th class="p-4">姓名</th><th class="p-4">電話</th><th class="p-4">註冊時間</th></tr></thead><tbody>\'+us+\'</tbody></table></div>\'}}async function uo(id,s){if(!confirm(\'確認更新狀態？\'))return;const r=await fetch(\'/api/admin/adminUpdateOrder\',{method:\'POST\',headers:{\'X-Admin-User\':u,\'X-Admin-Pass\':p,\'Content-Type\':\'application/json\'},body:JSON.stringify({action:\'adminUpdateOrder\',data:{orderId:id,status:s}})});if(r.ok)rf()}async function sc(id){const p1=document.getElementById(\'p-\'+id).value,c1=document.getElementById(\'c-\'+id).value;const r=await fetch(\'/api/admin/adminUpdateCourse\',{method:\'POST\',headers:{\'X-Admin-User\':u,\'X-Admin-Pass\':p,\'Content-Type\':\'application/json\'},body:JSON.stringify({action:\'adminUpdateCourse\',data:{id:id,price:p1,category:c1}})});if(r.ok){alert(\'成功\');rf()}}async function rf(){const r=await fetch(\'/api/admin/adminGetData\',{headers:{\'X-Admin-User\':u,\'X-Admin-Pass\':p}});const j=await r.json();sd=j;st(document.querySelector(\'.nav-active\').id.replace(\'n-\',\'\'))}</script></body></html>';
  return new Response(html, { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
}
