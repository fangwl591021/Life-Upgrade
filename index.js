...
export default {
  async fetch(request, env, ctx) {
    ...
    // 1. 專屬 Endpoint 分流 (確保 LIFF 請求絕對不進入登入或狀態頁)
...
function handleStatusPage() {
  const h = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>人生進化 Action</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-slate-50 flex items-center justify-center min-h-screen font-sans"><div class="max-w-md w-full bg-white p-12 rounded-[2.5rem] shadow-xl text-center border border-slate-100"><h1 class="text-2xl font-bold text-slate-800 mb-6 tracking-tight">人生進化 Action</h1><p class="text-slate-500 mb-8">系統服務正常運作中。</p><a href="/admin" class="block bg-blue-600 text-white py-4 rounded-xl font-semibold shadow-lg transition hover:bg-blue-700">進入管理系統</a></div></body></html>';
...
async function handleAdminPage(env) {
  const lines = [
    ...
    '<div id="login-box" class="fixed inset-0 bg-slate-50 flex items-center justify-center z-[70] transition-opacity">',
    '<div class="w-full max-w-sm bg-white p-10 rounded-3xl shadow-2xl border border-slate-100 text-center">',
    ...
    '<button onclick="st(\'orders\')" id="n-orders" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition font-medium text-base">📜 訂單管理</button>',
    '<button onclick="st(\'users\')" id="n-users" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition font-medium text-base">👥 會員中心</button>',
    ...
    'else{alert("帳號或密碼錯誤");}',
    '}catch(e){d.classList.remove("hidden"); d.innerText="GAS回傳格式錯誤，請確認 GAS 權限部署。\\n\\n內容："+t.substring(0,200);}',
    ...
    's.innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center">\'+\'<div class="text-slate-400 text-[10px] font-bold uppercase mb-2">實收累計</div><div class="text-2xl font-semibold text-slate-800">$\'+inc.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">待收總額</div><div class="text-2xl font-semibold text-slate-800">$\'+pend.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">預約筆數</div><div class="text-2xl font-semibold text-slate-800">\'+d.orders.length+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">總會員數</div><div class="text-2xl font-semibold text-slate-800">\'+d.users.length+\'</div></div></div>\'+\'<div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 font-medium"><h2 class="text-slate-800 mb-6 font-semibold border-l-4 border-blue-600 pl-4 tracking-tight uppercase">各課程營運數據統計</h2>\'+\'<div class="overflow-x-auto"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[11px] text-slate-500 uppercase font-bold tracking-widest border-b"><th class="p-4">課程名稱</th><th class="p-4 text-center">報名數</th><th class="p-4 text-emerald-600">已實收</th><th class="p-4 text-orange-600">待收款</th><th class="p-4">繳費狀況</th></tr></thead><tbody class="text-base text-slate-600">\' + d.courses.map(c=>{const ords=d.orders.filter(o=>o.courseName===c.name);return \'<tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors font-medium text-slate-700 text-base"><td class="p-4 font-semibold text-slate-800">\'+c.name+\'</td><td class="p-4 text-center font-semibold">\'+ords.length+\'</td><td class="p-4 text-emerald-600 font-semibold text-base">$\'+(ords.filter(o=>o.status==="已確認").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4 text-orange-500 font-semibold text-base">$\'+(ords.filter(o=>o.status==="待匯款").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4"><div class="flex items-center space-x-3 text-[11px] font-bold uppercase tracking-tighter text-slate-400">\'+\'<span class="text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded">付: \'+ords.filter(o=>o.status==="已確認").length+\'</span>\'+\'<span class="text-orange-500 bg-orange-50 px-2 py-0.5 rounded">待: \'+ords.filter(o=>o.status==="待匯款").length+\'</span></div></td></tr>\'}).join(\'\') + \'</tbody></table></div></div>\';',
    '} else if(State.tab==="courses"){',
    'let rows=d.courses.map(c=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700 font-sans"><td class="p-5 font-medium">\'+\'<div class="text-slate-800 text-base leading-tight mb-1 font-semibold">\'+c.name+\'</div><div class="text-[10px] font-mono text-slate-300 uppercase tracking-widest">\'+c.id+\'</div></td>\'+\'<td class="p-5 text-center"><span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-bold uppercase">\'+c.category+\'</span></td>\'+\'<td class="p-5"><div class="text-xl font-bold text-emerald-500 font-mono tracking-tight text-center">$\'+c.price+\'</div></td><td class="p-5 text-right"><button class="text-slate-400 hover:text-blue-600 font-semibold transition px-6 py-2.5 border border-slate-100 rounded-xl text-xs uppercase bg-white">編輯</button></td></tr>\').join(\'\');',
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">課程資訊</th><th class="p-5 text-center">類別</th><th class="p-5 text-center">定價</th><th class="p-5 text-right font-semibold">管理</th></tr></thead><tbody>\'+rows+\'</tbody></table></div>\';',
    '} else if(State.tab==="orders"){',
    'let rows=d.orders.map(o=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700 font-sans"><td class="p-5 font-mono text-[10px] text-slate-300">\'+o.orderId+\'</td><td class="p-5">\'+\'<div class="font-semibold text-slate-800 text-base mb-1">\'+(o.name||\'訪客\')+\'</div><div class="text-[11px] text-slate-400 font-mono tracking-tighter">\'+(o.phone||\'-\')+\'</div></td>\'+\'<td class="p-5 text-sm text-slate-500 font-semibold">\'+o.courseName+\'</td>\'+\'<td class="p-5 font-bold text-blue-600 text-lg font-mono">$\'+o.amount+\'</td>\'+\'<td class="p-5 text-center"><span class="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase \'+(o.status==="已確認"?"bg-emerald-100 text-emerald-700":o.status==="待匯款"?"bg-orange-100 text-orange-700":"bg-slate-100 text-slate-400")+\'">\'+o.status+\'</span></td>\'+\'<td class="p-5 text-right"><button class="bg-slate-900 text-white px-6 py-3 rounded-2xl text-xs font-semibold shadow-sm hover:bg-black transition tracking-wider uppercase">維修</button></td></tr>\').join(\'\');',
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">ID</th><th class="p-5 font-semibold">學員</th><th class="p-5 font-semibold">課程</th><th class="p-5 font-semibold">金額</th><th class="p-5 text-center">狀態</th><th class="p-5 text-right font-semibold">管理</th></tr></thead><tbody class="text-sm">\'+rows+\'</tbody></table></div>\';',
    ...
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">真實姓名</th><th class="p-5">聯絡手機</th><th class="p-5">身分證/UID</th><th class="p-5 text-right">註冊日期</th></tr></thead><tbody>\'+rows+\'</tbody></table></div>\';',
    '}',
    '}',
    '</script></body></html>'
  ];
...
async function handleLiffPayment(orderId, env) {
  const lines = [
    ...
    '<div class="card"><div class="label">報名真實姓名</div><input type="text" id="name" placeholder="請輸入姓名" required><div class="label">聯絡手機</div><input type="tel" id="phone" placeholder="請輸入電話" required><div class="label">匯款後五碼</div><input type="number" id="last5" placeholder="請輸入後五碼" required></div>',
    ...
    'try{const oR=await fetch(gas+"?action=getUserOrders&lineUid="+uid).then(r=>r.json());const o=oR.data.find(x=>x.orderId===oid);if(!o){document.getElementById("loading").innerText="⚠️ 預約單號不存在或已失效";return}document.getElementById("d-oid").innerText=o.orderId; document.getElementById("d-name").innerText=o.courseName;',
    'if(o.status==="已回報匯款"||o.status==="已確認"){alert("此單已完成回報，請等候核對。"); liff.closeWindow(); return}document.getElementById("loading").style.display="none"; document.getElementById("payForm").style.display="block";}',
    'catch(e){document.getElementById("loading").innerText="連線異常，請稍後再試。";}});',
    ...
    'const r=await res.json(); if(r.status==="success"){alert("回報完成！管理員會儘速處理。"); liff.closeWindow();}else{alert("失敗："+r.message); document.getElementById("subBtn").disabled=false; document.getElementById("subBtn").innerText="確認送出回報";}};</script></body></html>'
  ];
  return new Response(lines.join('\n'), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
}
