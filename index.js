...
function handleStatusPage() {
  const h = [
    ...
    '<a href="/admin" class="block bg-blue-600 text-white py-4 rounded-xl font-medium shadow-lg transition hover:bg-blue-700 text-lg">登入管理系統</a>',
    '</div></body></html>'
  ].join('');
...
async function handleAdminPage(env) {
  const h = [
    ...
    '<div><label class="text-[11px] font-bold text-slate-400 uppercase ml-1">管理員帳號</label><input type="text" id="admin-user" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Username"></div>',
    '<div><label class="text-[11px] font-bold text-slate-400 uppercase ml-1">管理員密碼</label><input type="password" id="admin-pw" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Password"></div>',
    ...
    '}catch(e){d.classList.remove("hidden"); d.innerText="GAS 回傳格式不正確。內容首 200 字："+t.substring(0,200);}',
    ...
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden font-sans"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b border-slate-100"><tr><th class="p-5">ID</th><th class="p-5">學員資訊</th><th class="p-5">報名課程</th><th class="p-5">實收</th><th class="p-5 text-center">狀態</th><th class="p-5 text-right font-semibold">管理</th></tr></thead><tbody class="text-sm">\'+rows+\'</tbody></table></div>\';',
...
async function handleLiffPayment(orderId, env) {
  const h = [
    ...
    '<div class="card"><div class="label">報名姓名</div><input type="text" id="name" placeholder="請輸入姓名" required><div class="label">聯絡電話</div><input type="tel" id="phone" placeholder="請輸入電話" required><div class="label">帳號末五碼</div><input type="number" id="last5" placeholder="請輸入五碼" required></div>',
