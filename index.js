...
export default {
  async fetch(request, env, ctx) {
    ...
    // 5. 系統狀態 (根路徑)
    if (request.method === 'GET' && pathname === '/') return handleStatusPage();
...
async function handleAdminPage(env) {
  const h = '<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>管理後台 - 人生進化 Action</title><script src="https://cdn.tailwindcss.com"></script><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;font-size:16px}.nav-active{background:#eff6ff;color:#2563eb;font-weight:600}.m-ov{background:rgba(15,23,42,0.6);backdrop-filter:blur(8px)}::-webkit-scrollbar{width:6px}::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:10px}</style></head><body class="bg-slate-50 text-slate-800 min-h-screen flex"><div id="login-box" class="fixed inset-0 bg-slate-50 flex items-center justify-center z-[70]"><div class="w-full max-w-sm bg-white p-10 rounded-2xl shadow-xl border border-slate-100 text-center"><h1 class="text-2xl font-bold mb-1 tracking-tight">人生進化 Action</h1><p class="text-slate-400 text-sm mb-8 uppercase tracking-widest font-medium">Administrator Login</p><div class="space-y-4 text-left"><label class="text-[11px] font-bold text-slate-400 uppercase ml-1">帳號</label><input type="text" id="admin-user" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Username"><label class="text-[11px] font-bold text-slate-400 uppercase ml-1">密碼</label><input type="password" id="admin-pw" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Password"><button onclick="doLogin()" id="lbtn" class="w-full bg-blue-600 text-white p-4 rounded-xl font-bold shadow-lg transition mt-2">進入系統</button></div><div id="diag" class="hidden mt-6 p-4 bg-red-50 text-red-700 text-[10px] text-left rounded-lg font-mono overflow-auto max-h-40"></div></div></div><aside class="w-64 bg-white border-r border-slate-200 flex-none hidden md:flex flex-col"><div class="p-6 border-b border-slate-100"><h1 class="text-xl font-bold text-slate-800 tracking-tighter">人生進化 Action</h1></div><nav class="flex-1 p-4 space-y-1"><button onclick="st(\'dashboard\')" id="n-dashboard" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition">📊 營運統計</button><button onclick="st(\'courses\')" id="n-courses" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition">📖 課程管理</button><button onclick="st(\'orders\')" id="n-orders" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition">📜 訂單管理</button><button onclick="st(\'users\')" id="n-users" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition">👥 會員中心</button></nav><div class="p-6 border-t border-slate-100 text-center"><button onclick="location.reload()" class="text-xs text-slate-400 font-bold hover:text-red-500 uppercase transition tracking-wider">Sign Out</button></div></aside><main class="flex-1 flex flex-col min-w-0 overflow-hidden"><header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0"><div id="path-display" class="text-sm text-slate-400 font-medium">首頁 / 營運統計</div><div class="flex items-center space-x-3"><span class="text-sm font-semibold text-slate-700">管理員</span><div class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xs border border-blue-200 shadow-sm">A</div></div></header><section class="p-8 overflow-y-auto flex-1 bg-slate-50/30" id="main-section"></section></main><div id="mc" class="hidden fixed inset-0 flex items-center justify-center z-50 p-6 m-ov"><div class="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"><div class="p-6 border-b flex justify-between items-center bg-white"><h2 class="text-lg font-bold text-slate-800">編輯課程詳情</h2><button onclick="cm(\'c\')" class="text-slate-400 hover:text-slate-600 text-3xl font-light">&times;</button></div><div class="p-8 overflow-y-auto flex-1 grid grid-cols-1 md:grid-cols-2 gap-10 text-sm text-slate-600 font-medium"><div><h3 class="font-bold text-emerald-600 border-l-4 border-emerald-500 pl-3 mb-6">基本資訊</h3><div class="space-y-4"><div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">課程 ID</label><input id="mi-id" readonly class="w-full bg-slate-50 border p-3 rounded-xl text-slate-400 font-mono text-sm"></div><div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">課程名稱</label><input id="mi-name" class="w-full border p-3 rounded-xl font-medium text-slate-800 focus:border-blue-500 outline-none"></div><div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">類別</label><select id="mi-cat" class="w-full border p-3 rounded-xl outline-none"><option>一般課程</option><option>工作坊課程</option><option>體驗課</option></select></div></div></div><div><h3 class="font-bold text-blue-600 border-l-4 border-blue-500 pl-3 mb-6">定價設定</h3><div class="space-y-4"><div><label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">報名費用</label><input id="mi-price" type="number" class="w-full border p-3 rounded-xl font-bold text-emerald-600 text-lg"></div><button onclick="saveC()" class="w-full bg-slate-900 text-white p-4 rounded-xl font-semibold shadow-lg hover:bg-black transition uppercase tracking-widest mt-4">儲存設定</button></div></div></div></div></div><div id="mo" class="hidden fixed inset-0 flex items-center justify-center z-50 p-6 m-ov"><div class="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden"><div class="p-6 border-b flex justify-between items-center"><h2 class="text-lg font-bold text-slate-800">訂單狀態維修</h2><button onclick="cm(\'o\')" class="text-slate-400 hover:text-slate-600 text-2xl">&times;</button></div><div class="p-8 space-y-6 text-sm"><div><label class="text-[10px] font-bold text-slate-400 uppercase mb-2 block font-semibold">當前付款狀態</label><select id="mos" class="w-full border p-4 rounded-xl font-semibold"><option>待付款</option><option>已回報匯款</option><option>已確認</option><option>已取消</option></select></div><div><label class="text-[10px] font-bold text-slate-400 uppercase mb-2 block font-semibold">實收金額</label><input id="mor" type="number" class="w-full border p-4 rounded-xl font-bold text-emerald-600 text-xl"></div><button onclick="saveO()" class="w-full bg-blue-600 text-white p-5 rounded-xl font-semibold shadow-lg hover:bg-blue-700 transition">更新紀錄</button></div></div></div><div id="loader" class="hidden fixed inset-0 flex items-center justify-center z-[100] bg-white/60 backdrop-blur-sm"><div class="animate-spin rounded-full h-10 w-10 border-4 border-blue-600 border-t-transparent"></div></div><script>' + 
  ...
  'async function doLogin(){' +
    ...
    'try{' +
      ...
      '}catch(e){d.classList.remove("hidden"); d.innerText="GAS Error: 請確認部署為所有人存取且為新版本。\\n首 200 字：" + t.substring(0,200);}' +
  ...
  'function render(){' +
    ...
    'if(State.tab==="dashboard"){' +
      ...
      's.innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col">\'+\'<div class="text-slate-400 text-[10px] font-bold uppercase mb-2">實收累計</div><div class="text-2xl font-semibold text-slate-800">$\'+inc.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">待收總額</div><div class="text-2xl font-semibold text-slate-800">$\'+pend.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">有效預約</div><div class="text-2xl font-semibold text-slate-800">\'+d.orders.length+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">系統學員</div><div class="text-2xl font-semibold text-slate-800">\'+d.users.length+\'</div></div></div>\'+\'<div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100"><h2 class="font-semibold text-base text-slate-800 mb-6 border-l-4 border-blue-600 pl-4 uppercase">各課程營收狀態統計</h2>\'+\'<div class="overflow-x-auto"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[10px] text-slate-500 uppercase font-bold tracking-widest border-b border-slate-100"><th class="p-4 font-semibold">課程名稱</th><th class="p-4 text-center">報名數</th><th class="p-4 text-emerald-600">已實收</th><th class="p-4 text-orange-600">待收款</th><th class="p-4">繳費分佈</th></tr></thead><tbody class="text-sm font-medium text-slate-600 text-base">\' + d.courses.map(c=>{const ords=d.orders.filter(o=>o.courseName===c.name);return \'<tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors font-medium text-slate-700"><td class="p-4 font-semibold text-slate-800">\'+c.name+\'</td><td class="p-4 text-center font-semibold">\'+ords.length+\'</td><td class="p-4 text-emerald-600 font-semibold">$\'+(ords.filter(o=>o.status==="已確認").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4 text-orange-500 font-semibold">$\'+(ords.filter(o=>o.status==="待匯款").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4"><div class="flex items-center space-x-3 text-[11px] font-bold uppercase tracking-tighter text-slate-400">\'+\'<span class="text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded">Paid: \'+ords.filter(o=>o.status==="已確認").length+\'</span>\'+\'<span class="text-orange-500 bg-orange-50 px-2 py-0.5 rounded">Wait: \'+ords.filter(o=>o.status==="待匯款").length+\'</span></div></td></tr>\'}).join(\'\') + \'</tbody></table></div></div>\';' +
...
async function handleLiffPayment(orderId, env) {
  ...
  'liff.init({liffId:"2009130603-sXSzvlh2"}).then(async()=>{' + 
    ...
    'try{' + 
      ...
      'if(!o){document.getElementById("loading").innerText="⚠️ 預約單號不存在或已被取消";return}' + 
