...
function handleStatusPage() {
  const lines = [
    ...
    '<p class="text-slate-500 mb-8">系統服務正常運作中。</p>',
  ...
  return new Response(lines.join(''), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
}
...
async function handleAdminPage(env) {
  const lines = [
    ...
    '<h1 class="text-2xl font-semibold mb-1 tracking-tight">人生進化 Action</h1><p class="text-slate-400 text-sm mb-8 uppercase tracking-widest">Management</p>',
    '<div class="space-y-4 text-left">',
    '<label class="text-[11px] font-bold text-slate-400 uppercase ml-1">管理員帳號</label>',
    '<input type="text" id="admin-user" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Username">',
    '<label class="text-[11px] font-bold text-slate-400 uppercase ml-1">管理員密碼</label>',
    '<input type="password" id="admin-pw" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Password">',
    '<button onclick="doLogin()" id="lbtn" class="w-full bg-blue-600 text-white p-4 rounded-xl font-semibold shadow-lg transition mt-2 text-lg">進入管理系統</button>',
    ...
    '<button onclick="st(\'dashboard\')" id="n-dashboard" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition text-base font-medium">📊 營運統計</button>',
    '<button onclick="st(\'courses\')" id="n-courses" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition text-base font-medium">📖 課程管理</button>',
    '<button onclick="st(\'orders\')" id="n-orders" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition text-base font-medium">📜 訂單管理</button>',
    '<button onclick="st(\'users\')" id="n-users" class="flex items-center w-full p-3 text-slate-500 hover:bg-slate-50 rounded-xl transition text-base font-medium">👥 會員中心</button>',
    '</nav><div class="p-6 border-t border-slate-100 text-center"><button onclick="location.reload()" class="text-xs text-slate-400 font-bold hover:text-red-500 uppercase transition tracking-widest">Sign Out</button></div></aside>',
    ...
    'async function doLogin(){',
      ...
      'try{',
        ...
        '}catch(e){d.classList.remove("hidden"); d.innerText="連線異常，請確認 GAS 部署為新版本並開放權限。\\n\\n內容："+t.substring(0,200);}',
    ...
    'function render(){',
      ...
      'if(State.tab==="dashboard"){',
        ...
        's.innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center">\'+\'<div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">實收金額</div><div class="text-2xl font-semibold text-slate-800">$\'+inc.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">待收總額</div><div class="text-2xl font-semibold text-slate-800">$\'+pend.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">預約總數</div><div class="text-2xl font-semibold text-slate-800">\'+d.orders.length+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">會員人數</div><div class="text-2xl font-semibold text-slate-800">\'+d.users.length+\'</div></div></div>\'+\'<div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 font-medium"><h2 class="text-slate-800 mb-6 font-semibold border-l-4 border-blue-600 pl-4 tracking-tight uppercase">各課程營收狀態</h2>\'+\'<div class="overflow-x-auto"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[11px] text-slate-500 uppercase font-bold tracking-widest border-b"><th class="p-4">課程名稱</th><th class="p-4 text-center">報名數</th><th class="p-4 text-emerald-600">已實收</th><th class="p-4 text-orange-600">待收款</th></tr></thead><tbody class="text-sm font-medium text-slate-600">\' + d.courses.map(c=>{const ords=d.orders.filter(o=>o.courseName===c.name);return \'<tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors"><td class="p-4 font-semibold text-slate-800">\'+c.name+\'</td><td class="p-4 text-center">\'+ords.length+\'</td><td class="p-4 text-emerald-600">$\'+(ords.filter(o=>o.status==="已確認").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4 text-orange-500">$\'+(ords.filter(o=>o.status==="待匯款").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td></tr>\'}).join(\'\') + \'</tbody></table></div></div>\';',
      '} else if(State.tab==="courses"){',
        'let rows=d.courses.map(c=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700"><td class="p-5">\'+\'<div class="text-slate-800 font-semibold mb-0.5">\'+c.name+\'</div><div class="text-[10px] font-mono text-slate-300 uppercase tracking-widest">\'+c.id+\'</div></td>\'+\'<td class="p-5"><span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-bold uppercase">\'+c.category+\'</span></td>\'+\'<td class="p-5"><div class="text-xl font-semibold text-emerald-500 tracking-tight">$\'+c.price+\'</div></td><td class="p-5 text-right"><button class="text-slate-400 hover:text-blue-600 font-semibold transition px-4 py-2 border border-slate-100 rounded-xl text-xs uppercase bg-white shadow-sm">編輯</button></td></tr>\').join(\'\');',
        's.innerHTML=\'<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">課程資訊</th><th class="p-5">類別</th><th class="p-5">定價</th><th class="p-5 text-right">管理</th></tr></thead><tbody>\'+rows+\'</tbody></table></div>\';',
      '} else if(State.tab==="orders"){',
        'let rows=d.orders.map(o=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700"><td class="p-5 font-mono text-xs text-slate-300">\'+o.orderId+\'</td><td class="p-5">\'+\'<div class="font-semibold text-slate-800 mb-0.5">\'+(o.name||\'訪客\')+\'</div><div class="text-[11px] text-slate-400 font-mono tracking-tighter">\'+(o.phone||\'-\')+\'</div></td>\'+\'<td class="p-5 text-sm text-slate-500">\'+o.courseName+\'</td>\'+\'<td class="p-5 font-semibold text-blue-600 font-mono tracking-tighter">$\'+o.amount+\'</td>\'+\'<td class="p-5"><span class="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase \'+(o.status==="已確認"?"bg-emerald-100 text-emerald-700":o.status==="待匯款"?"bg-orange-100 text-orange-700":"bg-slate-100 text-slate-400")+\'">\'+o.status+\'</span></td>\'+\'<td class="p-5 text-right"><button class="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-xs font-semibold shadow-sm hover:bg-black transition tracking-wider uppercase">維修</button></td></tr>\').join(\'\');',
        's.innerHTML=\'<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">ID</th><th class="p-5">學員資訊</th><th class="p-5">課程</th><th class="p-5">金額</th><th class="p-5">狀態</th><th class="p-5 text-right">操作</th></tr></thead><tbody class="text-sm">\'+rows+\'</tbody></table></div>\';',
      '} else if(State.tab==="users"){',
        'let rows=d.users.map(u=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700"><td class="p-5 font-semibold text-slate-800">\'+u.name+\'</td>\'+\'<td class="p-5 font-mono text-slate-600">\'+u.phone+\'</td>\'+\'<td class="p-5 text-right text-slate-400 text-xs font-semibold uppercase">\'+u.time+\'</td></tr>\').join(\'\');',
        's.innerHTML=\'<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">姓名</th><th class="p-5">聯絡手機</th><th class="p-5 text-right">註冊日期</th></tr></thead><tbody>\'+rows+\'</tbody></table></div>\';',
      '}',
    '}',
...
async function handleLiffPayment(orderId, env) {
  const lines = [
    ...
    '<body><div class="header">回報匯款資訊</div><div class="container"><div id="loading" style="text-align:center;padding:50px;color:#64748b text-base">正在讀取預約資料...</div><form id="payForm" style="display:none">',
    '<div class="card"><div class="label">預約單號</div><div id="d-oid" style="font-weight:semibold;color:#1e293b;font-size:18px;font-family:monospace"></div><div class="label" style="margin-top:16px">預約課程</div><div id="d-name" style="font-size:16px;color:#475569;font-weight:semibold"></div></div>',
    ...
    'const r=await res.json(); if(r.status==="success"){alert("回報成功！"); liff.closeWindow();}else{alert("失敗："+r.message); document.getElementById("subBtn").disabled=false;}};</script></body></html>'
  ];
  return new Response(lines.join('\n'), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
}
