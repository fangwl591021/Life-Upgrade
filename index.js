      ...
      <div id="app-box" class="hidden">
        <header class="bg-white border-b sticky top-0 px-4 h-16 flex items-center justify-between z-10">
          ...
          <nav class="flex space-x-6">
            ...
            <button onclick="st('courses')" id="n-courses" class="text-slate-500 text-sm py-5">課程維護</button>
            <button onclick="st('users')" id="n-users" class="text-slate-500 text-sm py-5">學員名單</button>
          </nav>
          <button onclick="location.reload()" class="text-xs text-slate-400">登出</button>
        </header>
        <main class="max-w-7xl mx-auto p-6" id="cnt"></main>
      </div>
      <script>
        let u="", p="", sd=null;
