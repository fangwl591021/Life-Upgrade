...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/api/admin/')) {
      ...
      try {
        // 使用 redirect: 'follow' 確保抓到 GAS 最終回傳的內容
    ...
    <body class="text-slate-800">
      <div id="login-screen" class="fixed inset-0 bg-slate-50 flex items-center justify-center z-50">
        <div class="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
          ...
          <p class="text-slate-500 text-sm mb-6 text-center">後台管理系統</p>
          <div class="space-y-4">
            ...
            <button onclick="doLogin()" id="login-btn" class="w-full bg-blue-600 text-white p-4 rounded-xl font-bold hover:bg-blue-700 transition">登入系統</button>
          </div>
          <div id="diag-box" class="hidden mt-6 p-4 bg-red-50 text-red-600 text-[10px] font-mono rounded-lg overflow-auto max-h-40 border border-red-100"></div>
        </div>
      </div>
