...
function handleStatusPage() {
  const lines = [
    ...
    '<p class="text-slate-500 mb-8 text-base">系統服務運作中，管理請進入後台。</p>',
...
async function handleAdminPage(env) {
  const lines = [
    ...
    '<title>人生進化 Action 管理後台</title><script src="https://cdn.tailwindcss.com"></script>',
    ...
    '<h1 class="text-2xl font-semibold mb-1 tracking-tight">人生進化 Action</h1><p class="text-slate-400 text-sm mb-8 uppercase tracking-widest font-medium">Administrator Portal</p>',
    '<div class="space-y-4 text-left">',
    '<label class="text-[11px] font-bold text-slate-400 uppercase ml-1">帳號</label>',
    '<input type="text" id="admin-user" class="w-full border border-slate-200 p-4 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition text-base" placeholder="Admin Username">',
    '<label class="text-[11px] font-bold text-slate-400 uppercase ml-1">密碼</label>',
    ...
    '<button onclick="doLogin()" id="lbtn" class="w-full bg-blue-600 text-white p-4 rounded-xl font-semibold shadow-lg transition mt-2 text-lg">進入系統</button>',
    ...
    'async function doLogin(){',
      ...
      'try{',
        ...
        '}catch(e){d.classList.remove("hidden"); d.innerText="連線成功但格式錯誤。請確認 GAS 部署權限。\\n\\n內容："+t.substring(0,200);}',
    ...
    'function render(){',
      ...
      'if(State.tab==="dashboard"){',
        ...
        's.innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center">\'+\'<div class="w-10 h-10 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-600 mb-3 font-bold">✓</div><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">實收金額</div><div class="text-2xl font-semibold text-slate-800">$\'+inc.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">待收總額</div><div class="text-2xl font-semibold text-slate-800">$\'+pend.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">預約數</div><div class="text-2xl font-semibold text-slate-800">\'+d.orders.length+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[11px] font-bold uppercase mb-2 tracking-widest">總會員</div><div class="text-2xl font-semibold text-slate-800">\'+d.users.length+\'</div></div></div>\'+\'<div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 font-medium"><h2 class="text-slate-800 mb-6 font-semibold border-l-4 border-blue-600 pl-4 tracking-tight uppercase">課程營運概覽</h2>\'+\'<div class="overflow-x-auto"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[11px] text-slate-500 uppercase font-bold tracking-widest border-b"><th class="p-4">課程名稱</th><th class="p-4 text-center">報名數</th><th class="p-4 text-emerald-600">已實收</th><th class="p-4 text-orange-600">待收款</th></tr></thead><tbody class="text-sm font-medium text-slate-600">\' + d.courses.map(c=>{const ords=d.orders.filter(o=>o.courseName===c.name);return \'<tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors font-medium text-slate-700"><td class="p-4 font-semibold text-slate-800 text-base">\'+c.name+\'</td><td class="p-4 text-center font-semibold">\'+ords.length+\'</td><td class="p-4 text-emerald-600 font-semibold">$\'+(ords.filter(o=>o.status==="已確認").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4 text-orange-500 font-semibold">$\'+(ords.filter(o=>o.status==="待匯款").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td></tr>\'}).join(\'\') + \'</tbody></table></div></div>\';',
      '} else if(State.tab==="courses"){',
        'let rows=d.courses.map(c=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700"><td class="p-5 font-medium">\'+\'<div class="text-slate-800 text-base leading-tight mb-1 font-semibold">\'+c.name+\'</div><div class="text-[10px] font-mono text-slate-300 uppercase tracking-widest">\'+c.id+\'</div></td>\'+\'<td class="p-5"><span class="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-bold uppercase tracking-widest">\'+c.category+\'</span></td>\'+\'<td class="p-5"><div class="text-xl font-bold text-emerald-500 font-mono tracking-tight">$\'+c.price+\'</div></td><td class="p-5 text-right"><button class="text-slate-400 hover:text-blue-600 font-semibold transition px-6 py-2.5 border border-slate-100 rounded-xl text-xs uppercase bg-white">編輯</button></td></tr>\').join(\'\');',
      ...
      '} else if(State.tab==="orders"){',
        'let rows=d.orders.map(o=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700"><td class="p-5 font-mono text-[10px] text-slate-300">\'+o.orderId+\'</td><td class="p-5">\'+\'<div class="font-semibold text-slate-800 text-base mb-1">\'+(o.name||\'訪客\')+\'</div><div class="text-[10px] text-slate-400 font-mono tracking-tighter">\'+(o.phone||\'-\')+\'</div></td>\'+\'<td class="p-5 text-sm text-slate-500 font-semibold">\'+o.courseName+\'</td>\'+\'<td class="p-5 font-bold text-blue-600 text-lg font-mono tracking-tighter">$\'+o.amount+\'</td>\'+\'<td class="p-5"><span class="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase \'+(o.status==="已確認"?"bg-emerald-100 text-emerald-700":o.status==="待匯款"?"bg-orange-100 text-orange-700":"bg-slate-100 text-slate-400")+\'">\'+o.status+\'</span></td>\'+\'<td class="p-5 text-right"><button class="bg-slate-900 text-white px-6 py-3 rounded-2xl text-xs font-semibold shadow-sm hover:bg-black transition tracking-wider uppercase">維修</button></td></tr>\').join(\'\');',
        's.innerHTML=\'<div class="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">ID</th><th class="p-5">學員</th><th class="p-5">課程</th><th class="p-5">金額</th><th class="p-5">狀態</th><th class="p-5 text-right">管理</th></tr></thead><tbody class="text-sm">\'+rows+\'</tbody></table></div>\';',
      '} else if(State.tab==="users"){',
        ...
        's.innerHTML=\'<div class="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b"><tr><th class="p-5">姓名</th><th class="p-5">手機</th><th class="p-5">ID/UID</th><th class="p-5 text-right">註冊日期</th></tr></thead><tbody>\'+rows+\'</tbody></table></div>\';',
      '}',
    '}',
    '</script></body></html>'
  ];
...
async function handleLiffPayment(orderId, env) {
  const lines = [
    ...
    '<div class="card"><div class="label">報名姓名</div><input type="text" id="name" placeholder="請輸入真實姓名" required><div class="label">聯絡電話</div><input type="tel" id="phone" placeholder="請輸入聯絡電話" required><div class="label">帳號末五碼</div><input type="number" id="last5" placeholder="請輸入帳號末五碼" required></div>',
    ...
    'try{const oR=await fetch(gas+"?action=getUserOrders&lineUid="+uid).then(r=>r.json());const o=oR.data.find(x=>x.orderId===oid);if(!o){document.getElementById("loading").innerText="⚠️ 單號不存在";return}document.getElementById("d-oid").innerText=o.orderId; document.getElementById("d-name").innerText=o.courseName;',
