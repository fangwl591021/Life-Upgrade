...
function handleStatusPage() {
  const h = [
    ...
    '<a href="/admin" class="block bg-blue-600 text-white py-4 rounded-xl font-semibold shadow-lg transition hover:bg-blue-700">後台登入</a>',
    '</div></body></html>'
  ].join('');
...
async function handleAdminPage(env) {
  const h = [
    ...
    '<h1 class="text-2xl font-bold mb-1">人生進化 Action</h1><p class="text-slate-400 text-sm mb-8 uppercase tracking-widest font-medium">Administrator</p>',
    ...
    '<div class="flex items-center space-x-3"><span class="text-sm font-semibold text-slate-700 uppercase">Admin</span>',
    ...
    'else{alert("帳號或密碼錯誤");}',
    '}catch(e){d.classList.remove("hidden"); d.innerText="收到無效資料。請檢查 GAS 是否部署為「所有人」並更新版本。\\n內容："+t.substring(0,200);}',
    ...
    's.innerHTML=\'<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center">\'+\'<div class="text-slate-400 text-[10px] font-bold uppercase mb-2">實收累計</div><div class="text-2xl font-semibold text-slate-800">$\'+inc.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">待收總額</div><div class="text-2xl font-semibold text-slate-800">$\'+pend.toLocaleString()+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">預約筆數</div><div class="text-2xl font-semibold text-slate-800">\'+d.orders.length+\'</div></div>\'+\'<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center"><div class="text-slate-400 text-[10px] font-bold uppercase mb-2">註冊會員</div><div class="text-2xl font-semibold text-slate-800">\'+d.users.length+\'</div></div></div>\'+\'<div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 font-medium font-sans text-base"><h2 class="text-slate-800 mb-6 font-semibold border-l-4 border-blue-600 pl-4 uppercase">營運數據概覽</h2>\'+\'<div class="overflow-x-auto"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-[10px] text-slate-500 uppercase font-bold tracking-widest border-b border-slate-100">\'+\'<th class="p-4">課程名稱</th><th class="p-4 text-center">報名數</th><th class="p-4 text-emerald-600">已實收</th><th class="p-4 text-orange-600">待收款</th><th class="p-4">狀況</th></tr></thead><tbody class="text-sm font-medium text-slate-600">\' + d.courses.map(c=>{const ords=d.orders.filter(o=>o.courseName===c.name);return \'<tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors font-medium text-slate-700 text-base"><td class="p-4 font-semibold text-slate-800 text-base">\'+c.name+\'</td><td class="p-4 text-center font-semibold">\'+ords.length+\'</td><td class="p-4 font-bold text-emerald-600">$\'+(ords.filter(o=>o.status==="已確認").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4 font-bold text-orange-500">$\'+(ords.filter(o=>o.status==="待匯款").reduce((s,o)=>s+(parseInt(o.amount)||0),0)).toLocaleString()+\'</td><td class="p-4 text-xs font-bold text-slate-400">\'+\'P:\'+ords.filter(o=>o.status==="已確認").length+\' / W:\'+ords.filter(o=>o.status==="待匯款").length+\'</td></tr>\'}).join(\'\') + \'</tbody></table></div></div>\';',
    '} else if(State.tab==="courses"){',
    'let rows=d.courses.map(c=>{const ords=d.orders.filter(o=>o.courseName===c.name); return \'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700 font-sans"><td class="p-5 font-medium">\'+\'<div class="text-slate-800 text-base mb-1 font-semibold">\'+c.name+\'</div><div class="text-[10px] font-mono text-slate-300 uppercase tracking-widest">\'+c.id+\'</div></td>\'+\'<td class="p-5"><span class="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-bold uppercase tracking-widest">\'+c.category+\'</span></td>\'+\'<td class="p-5"><div class="text-xl font-bold text-emerald-500 font-mono tracking-tight text-base text-center">$\'+c.price+\'</div></td><td class="p-5 text-right"><button class="text-slate-400 hover:text-blue-600 font-semibold transition px-4 py-2 border border-slate-100 rounded-xl text-xs uppercase bg-white">編輯</button></td></tr>\'}).join(\'\');',
    ...
    'let rows=d.orders.map(o=>\'<tr class="hover:bg-slate-50 transition border-b border-slate-100 text-base font-medium text-slate-700 font-sans"><td class="p-5 font-mono text-[10px] text-slate-300">\'+o.orderId+\'</td><td class="p-5">\'+\'<div class="font-semibold text-slate-800 text-base mb-1">\'+(o.name||\'訪客\')+\'</div><div class="text-[11px] text-slate-400 font-mono tracking-tighter">\'+(o.phone||\'-\')+\'</div></td>\'+\'<td class="p-5 text-sm text-slate-500 font-semibold">\'+o.courseName+\'</td>\'+\'<td class="p-5 font-bold text-blue-600 text-lg font-mono text-base">$\'+o.amount+\'</td>\'+\'<td class="p-5 text-center"><span class="px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-tight \'+(o.status==="已確認"?"bg-emerald-100 text-emerald-700":o.status==="待匯款"?"bg-orange-100 text-orange-700":"bg-slate-100 text-slate-400")+\'">\'+o.status+\'</span></td>\'+\'<td class="p-5 text-right"><button class="bg-slate-900 text-white px-6 py-3 rounded-2xl text-xs font-semibold shadow-sm hover:bg-black transition tracking-wider uppercase">維修</button></td></tr>\').join(\'\');',
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden font-sans"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b border-slate-100"><tr><th class="p-5 font-semibold text-base text-slate-400">單號</th><th class="p-5 font-semibold text-base text-slate-400">學員</th><th class="p-5 font-semibold text-base text-slate-400">課程</th><th class="p-5 font-semibold text-base text-slate-400">金額</th><th class="p-5 text-center text-base text-slate-400">狀態</th><th class="p-5 text-right font-semibold text-base text-slate-400">管理</th></tr></thead><tbody class="text-sm">\'+rows+\'</tbody></table></div>\';',
    ...
    's.innerHTML=\'<div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden font-sans"><table class="w-full text-left">\'+\'<thead class="bg-slate-50 text-[11px] text-slate-400 font-bold uppercase border-b border-slate-100"><tr><th class="p-5 text-base">真實姓名</th><th class="p-5 text-base">手機</th><th class="p-5 text-base">UID</th><th class="p-5 text-right font-semibold text-base">註冊日期</th></tr></thead><tbody>\'+rows+\'</tbody></table></div>\';',
    '}',
    '}',
    '</script></body></html>'
  ].join('\n');
...
async function handleLiffPayment(orderId, env) {
  const h = [
    ...
    '<div class="card"><div class="label">預約單號</div><div id="d-oid" style="font-weight:semibold;color:#1e293b;font-size:20px;font-family:monospace"></div><div class="label" style="margin-top:16px">課程名稱</div><div id="d-name" style="font-size:18px;color:#475569;font-weight:semibold"></div></div>',
    '<div class="card"><div class="label">真實姓名</div><input type="text" id="name" placeholder="請輸入姓名" required><div class="label">聯絡手機</div><input type="tel" id="phone" placeholder="請輸入手機" required><div class="label">匯款後五碼</div><input type="number" id="last5" placeholder="請輸入五碼" required></div>',
    ...
    'if(o.status==="已回報匯款"||o.status==="已確認"){alert("此單已回報完成。"); liff.closeWindow(); return}document.getElementById("loading").style.display="none"; document.getElementById("payForm").style.display="block";}',
    'catch(e){document.getElementById("loading").innerText="連線異常，請重新點選網址。";}});',
    ...
    'const r=await res.json(); if(r.status==="success"){alert("回報成功！我們會儘速審核。"); liff.closeWindow();}else{alert("失敗："+r.message); document.getElementById("subBtn").disabled=false; document.getElementById("subBtn").innerText="確認送出回報";}};</script></body></html>'
  ].join('\n');
  return new Response(h, { headers: { 'Content-Type': 'text/html;charset=UTF-8' } });
}
...
async function handleLiffDescription(cid, env) {
  const h = [
    ...
    '<body><div id="loading" style="padding:100px;text-align:center;color:#64748b;font-size:18px">正在載入詳情...</div><div id="app" style="display:none"><img id="c-img"><div class="content"><h1 id="c-name" style="font-size:24px;font-weight:bold;color:#1e293b;margin:0"></h1><div class="price" id="c-price"></div><div id="c-desc" class="desc"></div></div><div style="height:100px"></div></div><div class="btn-box" id="btn-c" style="display:none"><button class="btn" onclick="liff.closeWindow()">關閉詳情</button></div>',
    '<script src="https://static.line-scdn.net/liff/edge/2/sdk.js"></script>',
