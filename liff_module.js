...
export async function handleLiffPayment(orderId, env) {
  const h = [
    ...
    '<div class="card"><div class="label">報名姓名</div><input type="text" id="name" placeholder="請確認您的姓名" required><div class="label">聯絡電話</div><input type="tel" id="phone" placeholder="請確認您的電話" required><div class="label">匯款帳號後五碼</div><input type="number" id="last5" placeholder="請輸入後五碼" required></div>',
    ...
    'async function start(){',
      'try {',
        ...
        ']);',
        'const o = orderRes.data.find(x => x.orderId.toString() === targetOid.toString());',
        'if(!o){document.getElementById("loading").innerHTML="⚠️ 找不到此筆單號，請檢查連結是否正確。"; return}',
        'if(o.status==="已回報匯款"||o.status==="已確認"){alert("此單已回報完成。"); liff.closeWindow(); return}',
        'if(userRes.data){ document.getElementById("name").value = userRes.data.name || ""; document.getElementById("phone").value = userRes.data.phone || ""; }',
      ...
      '} catch(e) { document.getElementById("loading").innerText="載入異常，請重試。"; }',
    '}',
    'document.getElementById("payForm").onsubmit=async(e)=>{',
      ...
      'try{',
        ...
        'if(r.status==="success"){',
          '// 【核心更新】移除 alert，直接關閉 LIFF 視窗',
          'liff.closeWindow();',
        '}else{',
          'alert("回報失敗："+r.message); b.disabled=false; b.innerText="確認送出回報";',
        '}',
...
export async function handleLiffDescription(cid, env) {
  return new Response("Description Page Placeholder", { status: 200 });
}
