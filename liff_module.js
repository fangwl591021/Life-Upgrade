// LIFF 專屬模組 - 物理消除 iOS 彈窗提示並解決卡死問題
export async function handleLiffPayment(orderId, env) {
  const h = [
    ...
    '<div class="card"><div class="label">報名姓名</div><input type="text" id="name" required><div class="label">聯絡電話</div><input type="tel" id="phone" required><div class="label">匯款後五碼</div><input type="number" id="last5" placeholder="請輸入匯款帳號末五碼" required></div>',
    '<button type="submit" class="btn" id="subBtn">確認送出回報</button></form></div></div>',
    '<!-- 【修復】成功介面：引導手動返回以消除 iOS 系統警告 -->',
    '<div id="success-ui"><div class="success-icon">✅</div><h2 style="color:#1DB446;margin-bottom:8px">回報成功！</h2><p style="color:#64748b;margin-bottom:32px">感謝您的配合，我們將儘速處理。</p>',
    '<button onclick="liff.closeWindow()" class="btn" style="background:#1DB446">返回聊天室</button></div>',
