...
export async function handleLiffPayment(orderId, env) {
  const h = [
    ...
    'try{const oR = await fetch(gas + "?action=getUserOrders&lineUid=" + uid).then(r => r.json()); const o = oR.data.find(x => x.orderId === oid); if(!o){document.getElementById("loading").innerText="⚠️ 預約資料不存在"; return} document.getElementById("d-oid").innerText=o.orderId; document.getElementById("d-name").innerText=o.courseName; if(o.status==="已回報匯款"||o.status==="已確認"){alert("此單已回報完成"); liff.closeWindow(); return} document.getElementById("loading").style.display="none"; document.getElementById("payForm").style.display="block";}',
