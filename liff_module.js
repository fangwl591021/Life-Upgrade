...
export async function handleLiffPayment(orderId, env) {
  const h = [
    ...
    '.header{background:#1DB446;color:white;padding:30px 20px;text-align:center;font-weight:600;font-size:20px}',
    ...
    '.label{font-size:12px;font-weight:bold;color:#94a3b8;text-transform:uppercase;margin-bottom:6px;letter-spacing:1px}',
    '.val{font-size:18px;font-weight:600;color:#0f172a;margin-bottom:20px;font-family:monospace}',
    'input{width:100%;padding:16px;border:1.5px solid #e2e8f0;border-radius:16px;box-sizing:border-box;margin-bottom:20px;outline:none;font-size:16px;transition:all 0.2s}',
    '.btn{background:#007AFF;color:white;padding:20px;border-radius:20px;border:none;width:100%;font-size:18px;font-weight:600;cursor:pointer;box-shadow:0 10px 25px rgba(0,122,255,0.3)}',
    ...
    '<div class="label">聯絡電話</div><input type="tel" id="phone" placeholder="請輸入手機號碼" required>',
    '<div class="label">匯款帳號後五碼</div><input type="number" id="last5" placeholder="請輸入五碼" required></div>',
    ...
    'document.getElementById("payForm").onsubmit=async(e)=>{',
      ...
      'try{',
        ...
        'const r=await res.json(); if(r.status==="success"){alert("回報成功！我們會儘速審核。");liff.closeWindow();}else{alert("失敗："+r.message); b.disabled=false; b.innerText="送出回報資訊";}',
...
export async function handleLiffDescription(cid, env) {
  const h = [
    '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{font-family:sans-serif;margin:0;background:#fff;font-size:16px}img{width:100%;height:auto;background:#f8fafc;display:block}.content{padding:24px}.price{color:#ef4444;font-weight:600;font-size:26px;margin:12px 0}.desc{line-height:1.8;white-space:pre-wrap;font-size:16px;color:#334155}.btn-box{position:fixed;bottom:0;width:100%;padding:20px;background:rgba(255,255,255,0.9);backdrop-filter:blur(8px);border-top:1px solid #f1f5f9;box-sizing:border-box}.btn{background:#007AFF;color:#fff;padding:16px;border-radius:14px;width:100%;font-weight:600;font-size:18px;border:none;cursor:pointer}</style></head>',
    '<body><div id="loading" style="padding:100px;text-align:center;color:#64748b">載入中...</div><div id="app" style="display:none"><img id="c-img"><div class="content"><h1 id="c-name" style="font-size:24px;font-weight:bold;color:#1e293b;margin:0"></h1><div class="price" id="c-price"></div><div id="c-desc" class="desc"></div></div><div style="height:100px"></div></div><div class="btn-box" id="btn-c" style="display:none"><button class="btn" onclick="liff.closeWindow()">關閉詳情</button></div>',
    '<script src="https://static.line-scdn.net/liff/edge/2/sdk.js"></script>',
